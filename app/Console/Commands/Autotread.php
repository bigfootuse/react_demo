...
class Autotread extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $data1 = file_get_contents("https://api.coinmarketcap.com/v1/ticker/?limit=4");
            $data = json_decode($data1, TRUE);
            $setting = Setting::find(1);
            if ($data[1]["symbol"] == "ETH")
                $eth = $data[1]["price_usd"];
            elseif ($data[0]["symbol"] == "BCH")
        ...
        $withdraw_data = Withdraw::whereNull('txid')->get();

        foreach ($withdraw_data as $key_w)
        {
            if($key_w->withdraw_id !="" && $key_w->withdraw_id !="0"){
                $main_id=$key_w->withdraw_id;
                $result = $cp_helper->get_wdata($key_w->withdraw_id);

                if ($result['error'] == 'ok')
                {
                    if($result['result']['status']==2)
                    {
                        $ww=Withdraw::find($key_w->id);
                        $ww->txid=$result['result']['send_txid'];
                        $ww->save();
                    }
                }
                else
